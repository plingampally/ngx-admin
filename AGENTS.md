...
- Node **18.20.8** (`.nvmrc`, `.node-version`). Angular CLI 16 requires ^16.14 || ^18.10; Node 20+ is rejected by the install guard.
...
- `npm run lint`
- `docker build -f Dockerfile.ci -t ngx-admin-a16-ci .` — clean-room proof (install + build + test) on Node 18; CI gate only, not for dev.
...
- Lives in `playwright/` (the `e2e/` dir is dead Protractor). `playwright.config.ts` has **no `webServer` block** — always point `BASE_URL` at an already-running dev server:
  - `BASE_URL=http://localhost:4216 npm run e2e:pw` — run against this branch's server
  - `BASE_URL=http://localhost:4215 npm run e2e:pw` — run against the Angular 15 source (must be 91/91; it is what the goldens were captured from)
  - `npm run e2e:pw:update` — regenerate snapshots (`--update-snapshots`)
  - `npm run e2e:pw:report` — open the HTML report
- **Golden policy:** snapshots in `playwright/tests/__snapshots__/` are captured from the *previous* Angular version's server (`:4215` for the 16 step, recaptured on this machine because cross-machine font rasterization fails 4 typography shots) and committed. Never regenerate them from the server under test — the diff between goldens and the current version IS the migration evidence.
- First run needs `npx playwright install chromium`.
- 91 tests: 41 navigation, 4 smoke, 1 theme-cycle, 45 visual. Two routes carry tolerated known console errors (`/pages/dashboard` echarts `setOption`, `/pages/editors/ckeditor` CKEditor strict-mode) — pre-existing upstream bugs verified identical on Angular 14, 15 and 16; the routes still run and any *new* console error fails the test.
